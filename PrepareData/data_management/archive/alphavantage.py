...
"""
This module is for for fetching daily close price from the Alphavantage API and updating the daily data table in the database.
It is found that alphavantage API is performing pretty good and fast, hence this method remains unused now.
"""
import sys
...
def Daily(exchange=None):
    ...
    q = f"SELECT date FROM daily_data JOIN assets ON daily_data.id_asset = assets.id JOIN market ON assets.market = market.market_id WHERE market.market_short_name = '{exchange}' ORDER BY daily_data.date DESC LIMIT 1;"
    try:
...
def HistoryForAnAsset(asset):
    ...
    try:
        streaks = streaksLib.UpdateStreaksAllForSymbol(symbol, symbol_id=symbol_id)
        streak = streaksLib.UpdateStreakLatestForSymbol(symbol, symbol_id=symbol_id)
    except KeyboardInterrupt:
        logger.info("History update interrupted by user")
        sys.exit()
    except Exception as e:
        # failedSymbols.append(symbol)
        logger.warning("%s UpdateStreaksAllForSymbol: Failed for %s", e, symbol)
...
def History(exchange=None):
    ...
    with multiprocessing.Pool(processes=processesParallel) as pool:
        results = pool.map(HistoryForAnAsset, [asset for _, asset in assets.iterrows()])
    ...
    logger.info("%s Running streaks summary sent to db", exchange)
    # print("failedSymbols:", failedSymbols)
    logger.info("%s Finished data_update.History", exchange)
...
def AssetsForDailyUpdate(exchange):
    ...
    assetsWithDownStreak = dfBulk[dfBulkUpLogical.all(axis=1)]
